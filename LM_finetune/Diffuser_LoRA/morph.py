...
pipeline: InversionPipeline = InversionPipeline.from_pretrained(sd_path).to("cuda")
...
for i in range(n_frames + 1):
    ...
    output = pipeline(prompt=prompt, latents=latent, guidance_scale=1.0).images[0]
    images.append(output)

images[0].save(
    "output.gif", save_all=True, append_images=images[1:], duration=100, loop=0
)
