...
class DDPMScheduler(Scheduler):
    ...
    def step(
        ...
        denoised_image = (
            noisy_image - torch.sqrt(beta_bar_at_t) * noise_pred
        ) / torch.sqrt(alpha_bar_at_t)
        ...
        pred_original_sample_coeff = (
            torch.sqrt(alpha_bar_at_prev_t) * current_beta_t
        ) / beta_bar_at_t
        current_sample_coeff = (
            torch.sqrt(current_alpha_t) * beta_bar_at_prev_t / beta_bar_at_t
        )
        pred_prev_image = (
            pred_original_sample_coeff * denoised_image
