...
def hdr_plot_style():
    ...
    # Set colors cycle
    colors = mpl.cycler(
        'color', ['#3388BB', '#EE6666', '#9988DD', '#EECC55', '#88BB44', '#FFBBBB']
    )
...
def plot_boundary(W, iVal, style, fig):
    ...
    for i in range(nUnits):
        ...
        plt.plot(
            xLims,
            (-np.dot(W[i, 1], xLims) - W[i, 0]) / W[i, 2],
            linestyle=style,
            color=color,
            linewidth=1.5,
        )
...
def plot_data(X, y):
    ...
    plt.scatter(
        X[neg, 0], X[neg, 1], marker='o', edgecolor='k', linewidths=2, s=50, c=[1, 0, 0]
    )
    return fig
...
def plot_svc_decision_function(model, ax=None, plot_support=True):
    ...
    ax.contour(
        X, Y, P, colors='w', levels=[-1, 0, 1], alpha=0.9, linestyles=['--', '-', '--']
    )
...
def fit_multivariate_gaussian(X_s):
    ...
    pos[:, :, 0] = X
    pos[:, :, 1] = Y
    norm = distribution.MultivariateNormal(
        torch.Tensor(gmm.means_[0]), torch.Tensor(gmm.covariances_[0])
    )
...
def fit_gaussian_mixture(X_s):
    ...
    for i in range(4):
        norm = distribution.MultivariateNormal(
            torch.Tensor(gmm.means_[i]), torch.Tensor(gmm.covariances_[i])
        )
        Z += torch.exp(norm.log_prob(torch.Tensor(pos))).numpy()
    plt.figure(figsize=(10, 8))
    ax = plt.gca()
...
def plot_gmm(gmm, X, label=True, ax=None):
    ...
    if label:
        ax.scatter(
            X[:, 0], X[:, 1], c=labels, s=40, cmap='magma', edgecolor='gray', zorder=2
        )
    else:
