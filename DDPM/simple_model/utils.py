...
def make_beta_schedule(schedule='linear', n_steps=1000, start=1e-5, end=1e-2):
    if schedule == 'linear':
        return torch.linspace(start, end, n_steps)
    elif schedule == 'exp':
        return np.exp(torch.linspace(np.log(start), np.log(end), n_steps))
    elif schedule == 'quad':
        return torch.linspace(start**0.5, end**0.5, n_steps) ** 2
    elif schedule == 'sigmoid':
        return torch.sigmoid(torch.linspace(-6, 6, n_steps)) * (end - start) + start
    else:
...
def approx_standard_normal_cdf(x):
    return 0.5 * (
        1.0
        + torch.tanh(
            torch.tensor(np.sqrt(2.0 / np.pi)) * (x + 0.044715 * torch.pow(x, 3))
        )
    )
...
def loss_variational(
    ...
    true_mean, true_var = q_posterior_mean_variance(
        x_0,
        x_t,
        t,
        posterior_mean_coef_1,
    ...
    decoder_nll = -discretized_gaussian_log_likelihood(
        x_0, means=model_mean, log_scales=0.5 * model_var
    )
